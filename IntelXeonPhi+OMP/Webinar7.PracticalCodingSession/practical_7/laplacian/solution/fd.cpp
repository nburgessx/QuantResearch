...
int main(int argc,char** argv){
  ...
  // WARMUP
  fd12(in,out,dx,dy,dz,512,512,512);
...
#pragma omp parallel for proc_bind(spread) schedule(dynamic)
  for(int64_t iz=0;iz<nz;iz+=BLOCKZ){
    int64_t zend = MIN(iz+BLOCKZ,nz);
    for(int64_t iy=0;iy<ny;iy+=BLOCKY){
      int64_t yend = MIN(iy+BLOCKY,ny);
      for(int64_t ix=0;ix<nx;ix+=BLOCKX){
	int64_t xend = MIN(ix+BLOCKX,nx);
	
	for(int64_t izz=iz;izz<zend;izz++){
	  int64_t jz=get_stencil(nz,izz);
	  for(int64_t iyy=iy;iyy<yend;iyy++){
	    int64_t jy=get_stencil(ny,iyy);
#pragma omp simd       
	    for(int64_t ixx=ix;ixx<xend;ixx++){
	      int64_t jx=get_stencil(nx,ixx);
...
#pragma unroll(13)	
		...
		d2fdx2 += coeffs[jx][i+jx]*in[id(ixx+i,iyy,izz)]/(dx*dx);
	      }
	      /*Second order y-derivative.*/
#pragma unroll(13)	
	      for(int64_t i=-jy;i<=ORDER-jy ;i++){
		d2fdy2 += coeffs[jy][i+jy]*in[id(ixx,iyy+i,izz)]/(dy*dy);
	      }
	      /*Second order z-derivative.*/
#pragma unroll(13)	
	      for(int64_t i=-jz;i<=ORDER-jz ;i++){
		d2fdz2 += coeffs[jz][i+jz]*in[id(ixx,iyy,izz+i)]/(dz*dz);
	      }
	      
	      /*Approximate Laplacian.*/
	      out[id(ixx,iyy,izz)]=d2fdx2+d2fdy2+d2fdz2;
	    }
	  }
	}
      }
